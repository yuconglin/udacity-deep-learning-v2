...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        ### Backward pass ###
        if len(hidden_outputs.shape) == 1:
           hidden_outputs = hidden_outputs.reshape(-1, len(hidden_outputs))
        ...
        hidden_error = error.dot(self.weights_hidden_to_output.T) * hidden_outputs * (1 - hidden_outputs)
        
        # TODO: Backpropagated error terms - Replace these values with your calculations.
        output_error_term = hidden_outputs.T.dot(error)

        X = X.reshape(-1, len(X)) 
        hidden_error_term = X.T.dot(hidden_error)
